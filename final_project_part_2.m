...
% ecog = smoothdata(ecog, 'movmean', 200);
...
% train_ecog = train_ecog - mean(train_ecog, 2); % common reference averaging
% train_ecog = smoothdata(train_ecog, 'movmean', 200);
train_dg = dataglove(1:round(P * m), :);
val_ecog = ecog(round(P * m) + 1:end, :);
% val_ecog = val_ecog - mean(val_ecog, 2);
% val_ecog = smoothdata(val_ecog, 'movmean', 200);
...
% hold on
% plot(1:60000, val_dg(1:60000, 1), 'r')
% plot(1:60000, medfilt1(Y_hat_val_full(1:60000, 1), 500), 'b')
% plot(1:60000, Y_hat_val_full(1:60000, 1), 'g')
% hold off
% legend('True', 'Med. Prediction', 'Pred.');
% 
% ks = 1000:3000;
...
% for i = 1:length(ks)
%     k = ks(i);
%     test_2 = medfilt1(Y_hat_val_full, k);
%     corrs_store(i) = mean(diag(corr(test_2, val_dg)));
% end
% figure;
% plot(ks, corrs_store)

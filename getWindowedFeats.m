...
NumWins = @(xLen, fs, winLen, winDisp) ...
    floor((xLen - (winLen * fs))/(winDisp * fs));
num_wins = ...
    NumWins(size(clean_data, 1), fs, window_length, window_overlap);
% Then, loop through sliding windows
num_channels = size(clean_data, 2);
num_features = 6;
features = zeros(num_wins, num_channels * num_features,size(clean_data,3));
win_start_idx = 1;
for i = 1:num_wins
    ...
    curr_window = clean_data(win_start_idx:win_end_idx, :,:);
